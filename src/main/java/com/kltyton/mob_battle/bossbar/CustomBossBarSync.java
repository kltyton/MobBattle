...
public final class CustomBossBarSync {
    ...
    public static void add(ServerPlayer player, UUID bossBarUuid, Identifier styleId) {
    ...
    public static void remove(ServerPlayer player, UUID bossBarUuid) {
        if (ServerPlayNetworking.canSend(player, CustomBossBarPayload.ID)) {
            ServerPlayNetworking.send(player, new CustomBossBarPayload(bossBarUuid, Identifier.withDefaultNamespace("empty"), false));
        }
    }
}
