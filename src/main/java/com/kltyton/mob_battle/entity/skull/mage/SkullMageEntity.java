...
public class SkullMageEntity extends Skeleton implements GeoEntity, IModSkullEntity {
        ...
        this.entityData.get(MAGIC_PROJECTILE_ID).ifPresent(ref -> {
            ...
            if (projectile != null && projectile.isAlive()) {
                ...
                for (int i = 0; i < particles; i++) {
                    ...
                    if (i % 5 == 0) {
                        this.level().addParticle(
                                net.minecraft.core.particles.SpellParticleOption.create(ParticleTypes.INSTANT_EFFECT, 0xFFFFFF, 1.0F),
    ...
    protected AbstractArrow getArrow(ItemStack arrow, float damageModifier, @Nullable ItemStack shotFrom) {
        ...
        if (!this.level().isClientSide()) {
            this.entityData.set(MAGIC_PROJECTILE_ID, Optional.of(persistentProjectileEntity).map(EntityReference::of));
        }
    ...
    protected void addAdditionalSaveData(ValueOutput view) {
        ...
        view.putInt("SummonSkullCooldown", getSummonSkullCooldown());
    }
    ...
    protected void readAdditionalSaveData(ValueInput view) {
        ...
        setSummonSkullCooldown(Math.max(0, view.getIntOr("SummonSkullCooldown", getSummonSkullCooldown())));
    }
    @Override
    public void setOwner(@Nullable LivingEntity owner) {
        this.entityData.set(OWNER_UUID, Optional.ofNullable(owner).map(EntityReference::of));
    }
    @Override
