...
public class LobsterEntity extends Animal implements GeneralEntity<LobsterEntity> {
    ...
    public AgeableMob getBreedOffspring(ServerLevel world, AgeableMob mate) {
        if (mate instanceof LobsterEntity other
                ...
                && world.getRandom().nextFloat() < 0.02F) {
        ...
        if (mate instanceof LobsterEntity other && world.getRandom().nextFloat() < 0.5F) {
            child.setVariant(world.getRandom().nextBoolean() ? this.getVariant() : other.getVariant());
        } else {
